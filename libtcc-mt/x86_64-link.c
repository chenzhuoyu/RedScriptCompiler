...
int code_reloc (TCCState *s1, int reloc_type)
{
    switch (reloc_type) {
        case R_X86_64_32:
        case R_X86_64_32S:
    ...
    tcc_error (s1, "Unknown relocation type: %d", reloc_type);
    return -1;
}
...
/* Returns an enumerator to describe whether and when the relocation needs a
   ...
   different values. */
int gotplt_entry_type (TCCState *s1, int reloc_type)
{
    switch (reloc_type) {
        case R_X86_64_GLOB_DAT:
        case R_X86_64_JUMP_SLOT:
...
void relocate_init(TCCState *s1, Section *sr)
{
