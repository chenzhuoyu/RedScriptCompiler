...
ST_FUNC void tccelf_new(TCCState *s)
{
    ...
    s->common_section = new_section(s, ".common", SHT_NOBITS, SHF_PRIVATE);
    s->common_section->sh_num = SHN_COMMON;

    /* symbols are always generated for linking stage */
    s->symtab_section = new_symtab(s, ".symtab", SHT_SYMTAB, 0,
                                   ".strtab",
                                   ".hashtab", SHF_PRIVATE);
    s->symtab = s->symtab_section;
...
ST_FUNC void tccelf_bounds_new(TCCState *s)
{
    ...
    s->bounds_section = new_section(s, ".bounds",
                                    SHT_PROGBITS, SHF_ALLOC);
    s->lbounds_section = new_section(s, ".lbounds",
                                     SHT_PROGBITS, SHF_ALLOC);
}
#endif
...
{
    s->stab_section = new_section(s, ".stab", SHT_PROGBITS, 0);
    s->stab_section->sh_entsize = sizeof(Stab_Sym);
    s->stabstr_section = new_section(s, ".stabstr", SHT_STRTAB, 0);
    put_elf_str(s, s->stabstr_section, "");
    s->stab_section->link = s->stabstr_section;
    /* put first entry */
    put_stabs(s, "", 0, 0, 0, 0);
}

static void free_section(TCCState *s1, Section *s)
{
    tcc_free(s1, s->data);
}
...
ST_FUNC void tccelf_end_file(TCCState *s1)
{
    ...
    tr = tcc_mallocz(s1, nb_syms * sizeof *tr);
    ...
    for (i = 0; i < nb_syms; ++i) {
        ...
        tr[i] = set_elf_sym(s1, s, sym->st_value, sym->st_size, sym->st_info,
    ...
    for (i = 1; i < s1->nb_sections; i++) {
        ...
        if (sr->sh_type == SHT_RELX && sr->link == s) {
            ...
            for (; rel < rel_end; ++rel) {
                ...
                //if (n < 0) tcc_error(s1, "internal: invalid symbol index in relocation");
...
ST_FUNC Section *new_section(TCCState *s1, const char *name, int sh_type, int sh_flags)
{
    ...
    if (sh_flags & SHF_PRIVATE) {
        dynarray_add(s1, &s1->priv_sections, &s1->nb_priv_sections, sec);
    } else {
        sec->sh_num = s1->nb_sections;
        dynarray_add(s1, &s1->sections, &s1->nb_sections, sec);
    }
...
ST_FUNC void section_realloc(TCCState *s1, Section *sec, unsigned long new_size)
{
    ...
    while (size < new_size)
        size = size * 2;
    data = tcc_realloc(s1, sec->data, size);
...
ST_FUNC size_t section_add(TCCState *s1, Section *sec, addr_t size, int align)
{
    ...
    if (sec->sh_type != SHT_NOBITS && offset1 > sec->data_allocated)
        section_realloc(s1, sec, offset1);
...
{
    size_t offset = section_add(s1, sec, size, 1);
...
static void rebuild_hash(TCCState *s1, Section *s, unsigned int nb_buckets)
{
    ElfW(Sym) *sym;
    ...
    ptr = section_ptr_add(s1, s->hash, (2 + nb_buckets + nb_syms) * sizeof(int));
    ptr[0] = nb_buckets;
    ptr[1] = nb_syms;
    ptr += 2;
    hash = ptr;
...
ST_FUNC int put_elf_sym(TCCState *s1, Section *s, addr_t value, unsigned long size,
    int info, int other, int shndx, const char *name)
{
    ...
    sym = section_ptr_add(s1, s, sizeof(ElfW(Sym)));
    if (name && name[0])
        name_offset = put_elf_str(s1, s->link, name);
    else
        name_offset = 0;
    /* XXX: endianness */
    ...
    if (hs) {
        int *ptr, *base;
        ptr = section_ptr_add(s1, hs, sizeof(int));
        base = (int *)hs->data;
...
ST_FUNC int set_elf_sym(TCCState *s1, Section *s, addr_t value, unsigned long size,
                       int info, int other, int shndx, const char *name)
{
    ElfW(Sym) *esym;
...
#endif
                tcc_error_noabort(s1, "'%s' defined twice", name);
            }
        } else {
        do_patch:
    ...
    do_def:
        sym_index = put_elf_sym(s1, s, value, size,
...
/* put relocation */
ST_FUNC void put_elf_reloca(TCCState *s1, Section *symtab, Section *s, unsigned long offset,
                            int type, int symbol, addr_t addend)
{
    ...
    if (!sr) {
        ...
        /* if the symtab is allocated, then we consider the relocation
           are also */
        sr = new_section(s1, buf, SHT_RELX, symtab->sh_flags);
        sr->sh_entsize = sizeof(ElfW_Rel);
        sr->link = symtab;
        sr->sh_info = s->sh_num;
        s->reloc = sr;
    }
    rel = section_ptr_add(s1, sr, sizeof(ElfW_Rel));
    rel->r_offset = offset;
...
#else
    if (addend)
        tcc_error(s1, "non-zero addend on REL architecture");
#endif
}

ST_FUNC void put_elf_reloc(TCCState *s1, Section *symtab, Section *s, unsigned long offset,
                           int type, int symbol)
{
    put_elf_reloca(s1, symtab, s, offset, type, symbol, 0);
}
                      ...
                      unsigned long value)
{
    ...
    sym = section_ptr_add(s1, s1->stab_section, sizeof(Stab_Sym));
    if (str) {
        sym->n_strx = put_elf_str(s1, s1->stabstr_section, str);
    } else {
        sym->n_strx = 0;
    }
    sym->n_type = type;
                        ...
                        unsigned long value, Section *sec, int sym_index)
{
    ...
    put_elf_reloc(s1, s1->symtab_section, s1->stab_section,
                  s1->stab_section->data_offset - sizeof(unsigned int),
                  R_DATA_32, sym_index);
}
...
static void build_got(TCCState *s1)
{
    ...
    s1->got->sh_entsize = 4;
    set_elf_sym(s1, s1->symtab_section, 0, 4, ELFW(ST_INFO)(STB_GLOBAL, STT_OBJECT),
                0, s1->got->sh_num, "_GLOBAL_OFFSET_TABLE_");
    /* keep space for _DYNAMIC pointer and two dummy got entries */
    section_ptr_add(s1, s1->got, 3 * PTR_SIZE);
}
                                       ...
                                       int info, int sym_index)
{
    ...
    section_ptr_add(s1, s1->got, PTR_SIZE);
    ...
    if (need_plt_entry) {
        ...
        attr->plt_sym = put_elf_sym(s1, s1->symtab, attr->plt_offset, sym->st_size,
...
ST_FUNC void build_got_entries(TCCState *s1)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        for_each_elem(s, 0, rel, ElfW_Rel) {
            type = ELFW(R_TYPE)(rel->r_info);
            gotplt_entry = gotplt_entry_type(s1, type);
            sym_index = ELFW(R_SYM)(rel->r_info);
            sym = &((ElfW(Sym) *)s1->symtab_section->data)[sym_index];
...
/* put dynamic tag */
static void put_dt(TCCState *s1, Section *dynamic, int dt, addr_t val)
{
    ElfW(Dyn) *dyn;
    dyn = section_ptr_add(s1, dynamic, sizeof(ElfW(Dyn)));
    dyn->d_tag = dt;
    dyn->d_un.d_val = val;
}
...
static void add_init_array_defines(TCCState *s1, const char *section_name)
{
    ...
    set_elf_sym(s1, s1->symtab_section,
                0, 0,
...
ST_FUNC void tcc_add_bcheck(TCCState *s1)
{
    ...
    ptr = section_ptr_add(s1, s1->bounds_section, sizeof(*ptr));
    *ptr = 0;
    set_elf_sym(s1, s1->symtab_section, 0, 0,
                ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0,
                s1->bounds_section->sh_num, "__bounds_start");
    /* pull bcheck.o from libtcc1.a */
    sym_index = set_elf_sym(s1, s1->symtab_section, 0, 0,
                ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0,
                SHN_UNDEF, "__bound_init");
    if (s1->output_type != TCC_OUTPUT_MEMORY) {
        /* add 'call __bound_init()' in .init section */
        Section *init_section = find_section(s1, ".init");
        unsigned char *pinit = section_ptr_add(s1, init_section, 5);
        pinit[0] = 0xe8;
        write32le(pinit + 1, -4);
...
ST_FUNC void resolve_common_syms(TCCState *s1)
{
    ...
    for_each_elem(s1->symtab_section, 1, sym, ElfW(Sym)) {
	    ...
	    sym->st_value = section_add(s1, s1->bss_section, sym->st_size,
					sym->st_value);
        ...
        if (sym->st_shndx == SHN_UNDEF) {
            ...
            if (sym_index) {
                ...
                if ((type == STT_FUNC) || (type == STT_GNU_IFUNC)) {
                     ...
                     * address */
                    int dynindex
		      = put_elf_sym(s1, s1->dynsym, 0, esym->st_size,
				    ELFW(ST_INFO)(STB_GLOBAL,STT_FUNC), 0, 0,
				    name);
		    int index = sym - (ElfW(Sym) *) s1->symtab_section->data;
		    get_sym_attr(s1, index, 1)->dyn_index = dynindex;
                    ...
                    set_elf_sym(s1, s1->symtab, offset, esym->st_size,
                                 esym->st_info, 0, s1->bss_section->sh_num, name);
            ...
            name = (char *) s1->symtab_section->link->data + sym->st_name;
            set_elf_sym(s1, s1->dynsym, sym->st_value, sym->st_size, sym->st_info,
                        0, sym->st_shndx, name);
        }
    }
}
...
static void bind_libs_dynsyms(TCCState *s1)
{
    ...
    for_each_elem(s1->dynsymtab_section, 1, esym, ElfW(Sym)) {
        ...
        if (sym_index && sym->st_shndx != SHN_UNDEF
            && ELFW(ST_BIND)(sym->st_info) != STB_LOCAL) {
            set_elf_sym(s1, s1->dynsym, sym->st_value, sym->st_size,
...
static void tidy_section_headers(TCCState *s1, int *sec_order)
{
    ...
    snew = tcc_malloc(s1, s1->nb_sections * sizeof(snew[0]));
    backmap = tcc_malloc(s1, s1->nb_sections * sizeof(backmap[0]));
...
static int elf_output_file(TCCState *s1, const char *filename)
{
    ...
    if (dynamic) {
        ...
        if (s1->rpath)
            put_dt(s1, dynamic, s1->enable_new_dtags ? DT_RUNPATH : DT_RPATH,
                   put_elf_str(s1, dynstr, s1->rpath));
...
static void *load_data(TCCState *s1, int fd, unsigned long file_offset, unsigned long size)
{
    void *data;
                                ...
                                int fd, unsigned long file_offset)
{
    ...
    for(i = 1; i < nb_syms; i++, sym++) {
        ...
        /* add symbol */
        name = (char *) strtab + sym->st_name;
        sym_index = set_elf_sym(s1, s1->symtab_section, sym->st_value, sym->st_size,
...
ST_FUNC int tcc_load_dll(TCCState *s1, int fd, const char *filename, int level)
{
    ...
    for(i = 1, sym = dynsym + 1; i < nb_syms; i++, sym++) {
        ...
        set_elf_sym(s1, s1->dynsymtab_section, sym->st_value, sym->st_size,
...
static int ld_next(TCCState *s1, char *name, int name_size)
{
 ...
 redo:
    ...
    case '/':
        minp(s1);
        if (s1->ch == '*') {
            s1->file->buf_ptr = parse_comment(s1, s1->file->buf_ptr);
...
static inline int new_undef_syms(TCCState *s1)
{
    int ret = 0;
    ret = s1->new_undef_sym;
    s1->new_undef_sym = 0;
    return ret;
}
        ...
        if (!strcmp(filename, "AS_NEEDED")) {
            ...
            if (!as_needed) {
                ...
                if (group) {
                    ...
                    dynarray_add(s1, &libs, &nblibs, tcc_strdup(s1, filename));
                    if (libname[0] != '\0')
                        dynarray_add(s1, &libs, &nblibs, tcc_strdup(s1, libname));
                }
            }
        }
        t = ld_next(s1, filename, sizeof(filename));
